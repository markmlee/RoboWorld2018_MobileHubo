...
int RBFTSensor::RBFT_ReadData(void){
    ...
    if(SENSOR_TYPE == 0){
        ...
        canHandler->RBCAN_ReadData(&mb);
        if(mb.status != RBCAN_NODATA){
            dAccRoll =  (double)((int)((mb.data[1] | (mb.data[2]<<8))))*0.1;//(double)((short)((mb.data[0] | (mb.data[1]<<8))));
            dAccPitch = (double)((int)((mb.data[3] | (mb.data[4]<<8))))*0.1;//(double)((short)((mb.data[2] | (mb.data[3]<<8))));
